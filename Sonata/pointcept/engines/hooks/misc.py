...
class InformationWriter(HookBase):
    ...
    def after_step(self):
        self.mean_step_times = self.mean_step_times * (self.curr_iter - 1) / self.curr_iter + (time.time() - self.start_step_time) / self.curr_iter
        self.trainer.comm_info["iter_info"] += f"mean step time {self.mean_step_times:.2f}"
        ...
        peak_allocated = torch.cuda.max_memory_allocated(device) / (1024 ** 3)  # GB
        peak_reserved = torch.cuda.max_memory_reserved(device) / (1024 ** 3)    # GB
        self.trainer.comm_info["iter_info"] += f"Peak allocated memory: {peak_allocated:.2f} GB"
        self.trainer.comm_info["iter_info"] += f"Peak reserved memory: {peak_reserved:.2f} MB"
